...
class GSPHAccelerationWENO5Wang(GSPHAcceleration):
    ...
    def loop_all(self, d_idx, d_m, d_h, d_rho, d_cs, d_div, d_p, d_e, d_grhox,
        ...
        for i in range(N_NBRS):
            ...
            SPH_KERNEL.gradient(xij, rij, hj, dwj)
            SPH_KERNEL.gradient(xij, rij, 0.5 * (hi + hj), dwij)
            ...
            grhoi_dot_eij = (d_grhox[d_idx] * eij[0] +
                             d_grhoy[d_idx] * eij[1] +
                             d_grhoz[d_idx] * eij[2])
            grhoj_dot_eij = (s_grhox[s_idx] * eij[0] +
                             s_grhoy[s_idx] * eij[1] +
                             s_grhoz[s_idx] * eij[2])
            ...
            sstar = sp_vol[2]
            # Gradients in the local coordinate system
            rsi = (d_grhox[d_idx] * eij[0] +
                   d_grhoy[d_idx] * eij[1] +
                   d_grhoz[d_idx] * eij[2])
            psi = (d_px[d_idx] * eij[0] +
                   d_py[d_idx] * eij[1] +
                   d_pz[d_idx] * eij[2])
            vsi = (eij[0] * eij[0] * d_ux[d_idx] +
                   eij[0] * eij[1] * (d_uy[d_idx] + d_vx[d_idx]) +
            ...
            rsj = (s_grhox[s_idx] * eij[0] +
                   s_grhoy[s_idx] * eij[1] +
                   s_grhoz[s_idx] * eij[2])
            psj = (s_px[s_idx] * eij[0] +
                   s_py[s_idx] * eij[1] +
                   s_pz[s_idx] * eij[2])
            vsj = (eij[0] * eij[0] * s_ux[s_idx] +
                   eij[0] * eij[1] * (s_uy[s_idx] + s_vx[s_idx]) +
            ...
            # (85). The contribution due to \dot{x}^*_i will
            # be added in the integrator.
            vstardotdwi = (vstar[0] * dwi[0] +
                           vstar[1] * dwi[1] +
                           vstar[2] * dwi[2])
            vstardotdwj = (vstar[0] * dwj[0] +
                           vstar[1] * dwj[1] +
                           vstar[2] * dwj[2])
            d_ae[d_idx] += -mj * pstar * (vij_i * vstardotdwi +
                                          vij_j * vstardotdwj)
